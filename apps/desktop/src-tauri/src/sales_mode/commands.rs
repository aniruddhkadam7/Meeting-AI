...
pub async fn show_sales_overlay(app: AppHandle) -> Result<overlay_window::OverlayCaptureStatus, String> {
    run_on_main(&app, |app| {
        overlay_window::show_overlay_window(app, SALES_OVERLAY_LABEL, "Smallbird Sales — Overlay")
    })
    .await
}
...
pub async fn toggle_sales_overlay(app: AppHandle) -> Result<overlay_window::OverlayCaptureStatus, String> {
    run_on_main(&app, |app| {
        overlay_window::toggle_overlay_window(app, SALES_OVERLAY_LABEL, "Smallbird Sales — Overlay")
    })
    .await
}
