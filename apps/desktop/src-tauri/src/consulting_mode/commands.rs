...
pub async fn show_consulting_overlay(app: AppHandle) -> Result<overlay_window::OverlayCaptureStatus, String> {
    run_on_main(&app, |app| {
        overlay_window::show_overlay_window(app, CONSULTING_OVERLAY_LABEL, "Smallbird Consulting — Overlay")
    })
    .await
}
...
pub async fn toggle_consulting_overlay(app: AppHandle) -> Result<overlay_window::OverlayCaptureStatus, String> {
    run_on_main(&app, |app| {
        overlay_window::toggle_overlay_window(app, CONSULTING_OVERLAY_LABEL, "Smallbird Consulting — Overlay")
    })
    .await
}
