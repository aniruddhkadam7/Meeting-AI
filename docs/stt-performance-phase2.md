...
- `apps/desktop/src-tauri/src/hardware/stt_rag_coordination.rs` (new) —
  ...
  throttle stays active across Smallbird's two independently-lifecycled STT
...
reference-counted for Smallbird's two concurrent STT session types, never
...
Smallbird's shipped default configuration (all already the shipped defaults —
