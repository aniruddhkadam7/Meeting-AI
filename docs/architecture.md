...
Smallbird is a Windows desktop application that captures system audio during
...
- `GET /health` → `{"status": "ok", "service": "smallbird-backend"}`.
...
- **Document/knowledge-base state** lives entirely under
  `%APPDATA%\Smallbird\knowledge\` — raw files, extracted text, and
